...
while cap.isOpened():
    ...
    for detection in detections:
        ...
        if pose_results.pose_landmarks:
            ...
            step_width_pixels = abs(left_ankle[0] - right_ankle[0]) * width
            conversion_factor = 3779.5275591  # Exemplo: pixels por metro
            step_width = step_width_pixels / conversion_factor
            ...
            info_text = (
                ...
                f"Largura do passo: {step_width:.2f} m\n"
...
with open('analysis_results.csv', mode='w', newline='') as file:
    ...
    writer.writerow(["Tempo (s)", "Distância Percorrida (m)", "Comprimento do Passo (m)", "Ângulo Joelho Esq (°)", "Ângulo Joelho Dir (°)", "Comprimento da Passada (m)", "Largura"])
    writer.writerows(zip(timestamps, distances, step_lengths, left_knee_angles, right_knee_angles, stride_lengths, step_widths))
...
plt.plot(timestamps, step_widths, label="Largura do Passo (m)", color="green")
plt.xlabel("Tempo (s)")
plt.ylabel("Medições")
plt.title("Parâmetros da Marcha ao Longo do Tempo")
plt.legend()
